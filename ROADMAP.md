...
1. 장소명/POI -> 역명 resolver 추가
2. live 품질 평가 스크립트 추가
3. 역별 접근성 근거 세분화
4. hosted 운영 모드 설계와 배포 가이드
5. OAuth/OIDC gateway 전환
6. 실제 사용자 피드백 기반 문장 개선

## 1. 장소명/POI -> 역명 resolver 추가

목표: "코엑스", "서울역 KTX", "홍대", "고속터미널"처럼 역명이 아닌 표현도 교통약자 안내에 연결한다.

구현 범위:

- `app/data/place_aliases.yaml` 추가.
- `resolve_place_to_station(query: str)` 내부 helper 또는 MCP tool 후보를 설계한다.
- 장소명 하나가 여러 역 후보로 이어지면 판단하지 않고 선택 질문을 만든다.
- 후보에는 역명, 호선, station_id, 설명을 포함한다.

초기 데이터 후보:

- 코엑스 -> 삼성역, 봉은사역
- 서울역 KTX -> 서울역 1호선, 서울역 4호선
- 홍대 -> 홍대입구역
- 고속터미널 -> 고속터미널 3호선, 7호선, 9호선
- DDP -> 동대문역사문화공원 2호선, 4호선, 5호선

완료 기준:

- 장소명 입력이 "일치하는 역 없음"으로 바로 실패하지 않는다.
- 모호한 장소명은 후보를 제시한다.
- 확정 가능한 장소명은 기존 접근성 판단으로 연결된다.
